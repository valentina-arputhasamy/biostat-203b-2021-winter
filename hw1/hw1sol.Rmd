...
```{bash}
  for datafile in /usr/203b-data/mimic-iv/*/*.gz
    do 
      awk 'END { print NR }' $datafile
      done
```

The above code displays the number of lines in each data file in /usr/203b-data/mimic.iv that end with .gz. 
...
The above code also displays the number of lines in each data file in /usr/203b-data/mimic.iv that end with .gz, but unlike the previous example, it also includes the name of each file.
