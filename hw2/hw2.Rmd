...
- admission type (done)
- number of admissions per patient (done)
- admission location (done)
- discharge location  (done)
- insurance (done)
- language (done)  
- martial status (done)  
- ethnicity (done)
...
```{r}
admissions2 %>% ggplot(aes(subject_id)) +
  geom_bar(fill = "lightsteelblue2", color = "lightsteelblue4") + 
  coord_flip() + ggtitle("Number of Admits by Admission Type") + 
  xlab("Admission Type") + theme_minimal() 
```
...
```{r}
admissions2 <- admissions2 %>% mutate(language_recode = recode(language, "?" = "Unknown"))
...
```{r}
admissionyn <- mutate_all(replace(admissions2, is.na(admissions2$death_year), 0)) 
```
