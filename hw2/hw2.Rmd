...
```{r}
icustays %>% ggplot(aes(stayduration)) + geom_histogram(fill = "thistle4") + theme_bw() + ggtitle("ICU Stay Duration") + xlab("Length of ICU Stay \n (Log-Transformed)") + ylab("Count")
...
```{r}
ggplot() + 
  geom_boxplot(aes(y = icustays$stayduration)) + 
  scale_x_discrete( ) +
  labs(title = "ICU Unit Stay Duration in Days",
       y = "Number of Days")
```
...
```{r}
ggplot() + 
  geom_bar(aes(y = icustays$stayduration)) + scale_x_discrete( ) +
  labs(title = "ICU Unit Stay Duration in Days",
       y = "Number of Days")
```
...
```{r}
icustays %>% ggplot( aes(first_careunit)) + geom_bar() 
```
