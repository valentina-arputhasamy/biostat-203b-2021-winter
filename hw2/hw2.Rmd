...
- admission month day (done) 
- admission week day (done)
- admission hour (anything unusual?)  : not sure what he thinks is unusual
- number of deaths in each year  (done)
- admission type (done)
...
admissions %>% ggplot(aes(admit_year)) + geom_bar(binwidth = 1, fill = "lightsteelblue2", color = "lightsteelblue4") + ggtitle("Number of Admits by Year") + xlab("year") + theme_minimal()
```
...
```{r}
admissions2 %>% ggplot(aes(admit_month_recode)) + geom_bar(binwidth = 1, fill = "lightsteelblue2", color = "lightsteelblue4") + ggtitle("Number of Admits by Month") + xlab("month") + ylab("count") + scale_x_discrete() + theme_minimal() + ylim(c(0,45000))
```
...
admissions2 %>% ggplot(aes(admit_date)) + geom_bar(binwidth = 1, fill = "lightsteelblue2", color = "lightsteelblue4") + ggtitle("Number of Admits by Day of the Month") + xlab("Day of the Month")
```
...
admissions2 %>% ggplot(aes(admit_dow)) + geom_bar(binwidth = 1, fill = "lightsteelblue2", color = "lightsteelblue4") + ggtitle("Number of Admits by Day of the Week") + xlab("Day of the Week")
```
...
admissions2 %>% ggplot(aes(admit_hour)) + geom_bar(binwidth = 1, fill = "lightsteelblue2", color = "lightsteelblue4") + ggtitle("Number of Admits by Hour") + xlab("Hour") + theme_minimal() + scale_x_continuous(n.breaks = 10)
```
...
admissions2 %>% ggplot(aes(death_year)) + geom_bar(binwidth = 1, fill = "lightsteelblue2", color = "lightsteelblue4") + ggtitle("Number of Deaths per Year") + xlab("Year") + ylab("Number of Deaths") + theme_minimal()
```
...
```{r}
admissions2 %>% ggplot(aes(admission_type)) + geom_bar(fill = "lightsteelblue2", color = "lightsteelblue4") + coord_flip() + ggtitle("Number of Admits by Admission Type") + xlab("Admission Type") + theme_minimal() 
```
...
```{r}
patients %>% ggplot(aes(gender)) + geom_bar(fill = "lightsteelblue2", color = "lightsteelblue4") + coord_flip() + ggtitle("Number of Patients by Gender") + xlab("Gender") + theme_minimal() 
```
...
dictionary$itemid[dictionary$label %in% c("Heart Rate", "Arterial Blood Pressure systolic", "Arterial Blood Pressure mean", "Non Invasive Blood Pressure systolic", "Non Invasive Blood Pressure mean", "Temperature Celsius", "Respiratory Rate", "PAR-Oxygen saturation")]

chart_events.want <- chart_events %>% filter(V5 %in% c(220045, 220050, 220052, 220179, 220181, 220210, 223762, 228232))
